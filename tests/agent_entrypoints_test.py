...
""",
        ...
        self.write_fake(
            "git",
            f"""#!/bin/sh
if [ -n "${{FAKE_GIT_EXIT:-}}" ]; then
  exit "$FAKE_GIT_EXIT"
fi
case " $* " in
  *" status --porcelain "*)
    if [ -n "${{FAKE_GIT_STATUS_EXIT:-}}" ]; then
...
""",
    ...
    def entrypoint(
        self, name: str, *, cwd: Path | None = None, hk: bool = False, **extra: str
...
""",
        ...
        env = {
            ...
            "XDG_STATE_HOME": str(self.temp_path / "state"),
            "FAKE_CALL_LOG": str(self.calls),
            "FAKE_DOCTOR_JSON": str(self.doctor_json),
            "FAKE_MISE_STATE": str(self.mise_state),
            **extra,
        }
        return subprocess.run(
    ...
    def assert_message_schema(self, messages: list[dict[str, object]]) -> None:
        for item in messages:
            self.assertEqual(
                set(item),
                {"status", "stage", "code", "message", "next_command", "finding"},
            )
    ...
    def test_setup_portable_convergent_and_structured_commands(self) -> None:
        ...
        self.assert_command(
            commands["setup"], [str(self.repo.resolve() / ".agents" / "setup")]
        )
        self.assert_command(
            commands["doctor"],
            [
        ...
        self.assert_command(
            commands["focused"], ["python3", "tests/agent_entrypoints_test.py"]
        )
        self.assertIn(
            f"foreman cwd={self.repo.resolve()} args=--doctor --doctor-json --doctor-strict --repo {self.repo.resolve()}",
    ...
    def test_setup_failure_receipt_has_actionable_message(self) -> None:
        ...
        self.assert_command(
            error["next_command"], [str(self.repo.resolve() / ".agents" / "setup")]
        )
    ...
    def test_missing_mise_and_foreman_have_actionable_stderr_and_receipts(self) -> None:
        ...
        self.assert_command(
            self.messages(no_mise_receipt, "errors")[0]["next_command"],
            [str(self.repo.resolve() / ".agents" / "setup")],
        )
        ...
        self.assert_command(
            cast(dict[str, object], no_foreman_receipt["commands"])["install_local"],
            ["mise", "run", "install-local"],
        )
        self.assert_command(
            self.messages(no_foreman_receipt, "errors")[0]["next_command"],
            ["mise", "run", "install-local"],
        )
    ...
    def test_doctor_failure_and_malformed_valid_json_are_visible_errors(self) -> None:
        ...
        self.assertTrue(
            any(
                item["code"] == "doctor-finding"
                for item in self.messages(blocked_receipt, "errors")
            )
        )
        malformed_reports = (
            [],
            ...
            {"findings": {}},
            {**EMPTY_DOCTOR, "fixes": [{}]},
            {**EMPTY_DOCTOR, "fixes": [{**FIX, "provider": {}}]},
            {**EMPTY_DOCTOR, "findings": [{**FINDING, "provider": []}]},
            {
                **EMPTY_DOCTOR,
                "findings": [
                    {
                        ...
                        "severity": "error",
                        "area": "repo",
                        "summary": "missing canonical nullable fields",
                        "evidence": [],
                    }
                ],
            },
        ...
        for malformed in malformed_reports:
            ...
            for entrypoint in ("setup", "resume"):
                result = self.entrypoint(entrypoint)
                receipt = self.receipt(result)
                self.assertEqual(result.returncode, 1)
                self.assertIsNone(cast(dict[str, object], receipt["doctor"])["report"])
                self.assertTrue(
                    any(
                        item["code"] == "doctor-schema-invalid"
                        for item in self.messages(receipt, "errors")
                    )
                )
                self.assert_message_schema(self.messages(receipt, "errors"))
    ...
    def test_resume_read_only_reports_git_states_and_doctor_failure(self) -> None:
        ...
        self.assertTrue(
            any(
                item["code"] == "doctor-unavailable"
                for item in self.messages(detached, "errors")
            )
        )

    def test_log_resolution_matches_repo_relative_absolute_xdg_and_home_contract(
        self,
    ) -> None:
        cases = [
            ...
            (
                "FOREMAN_LOG_DIR",
                str(self.temp_path / "absolute state"),
                self.temp_path / "absolute state",
            ),
            (
                "XDG_STATE_HOME",
                "relative xdg",
                self.repo / "relative xdg" / "foreman" / "logs",
            ),
        ]
        for variable, value, directory in cases:
            ...
            report = self.receipt(
                self.entrypoint("resume", cwd=Path("/"), **{variable: value})
            )
            logs = cast(dict[str, object], report["logs"])
            expected_directory = (
                directory if Path(value).is_absolute() else directory.resolve()
            )
        ...
        empty_xdg_latest = self.repo / "foreman" / "logs" / "latest.log"
        empty_xdg_latest.parent.mkdir(parents=True)
        empty_xdg_latest.touch()
        report = self.receipt(
            self.entrypoint("resume", cwd=Path("/"), XDG_STATE_HOME="")
        )
        self.assertEqual(
            cast(dict[str, object], report["logs"])["latest"],
            str(empty_xdg_latest.resolve()),
        )

    def test_unavailable_repository_state_is_structured(self) -> None:
        result = self.entrypoint("resume", FAKE_GIT_EXIT="127")
        receipt = self.receipt(result)

        self.assertEqual(result.returncode, 0)
        repo = cast(dict[str, object], receipt["repo"])
        self.assertEqual(repo["status"], "metadata-unavailable")
        self.assertIsNone(repo["branch"])
        self.assertTrue(
            any(
                item["code"] == "repo-state-unavailable"
                for item in self.messages(receipt, "warnings")
            )
        )

    def test_failed_git_status_is_structured(self) -> None:
        result = self.entrypoint("resume", FAKE_GIT_STATUS_EXIT="5")
        receipt = self.receipt(result)
    ...
    def test_hk_success_and_failure_preserve_stderr(self) -> None:
        ...
        self.assertEqual(
            cast(
                dict[str, object],
                cast(dict[str, object], successful_receipt["optional_tools"])["hk"],
            )["status"],
            {"active_work": "fixture"},
        )
        ...
        self.assertTrue(
            any(
                item["code"] == "hk-status-unavailable"
                for item in self.messages(failed_receipt, "warnings")
            )
        )
