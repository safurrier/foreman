...
#[test]
fn bootstrap_prefers_codex_native_signal_and_falls_back_to_compatibility() {
    let fixture = TmuxFixture::new();
    let pane_id = fixture.new_session("alpha", &fixture.shell_command("Codex is planning a fix"));
    ...
    assert_eq!(first.codex_native.applied, 1);
    assert_eq!(first.codex_native.fallback_to_compatibility, 0);
    ...
    assert_eq!(second.codex_native.applied, 0);
    assert_eq!(second.codex_native.fallback_to_compatibility, 1);
}
...
"#,
    ...
    assert_eq!(agent.status, AgentStatus::Working);
    assert_eq!(summary.codex_native.applied, 0);
    assert_eq!(summary.codex_native.fallback_to_compatibility, 1);
    assert!(summary.codex_native.warnings.is_empty());
}

#[test]
fn binary_bootstrap_logs_codex_native_summary() {
    let fixture = TmuxFixture::new();
    let pane_id = fixture.new_session("alpha", &fixture.shell_command("Codex is planning a fix"));
    fixture.wait_for_capture(&pane_id, "Codex is planning");
    ...
    assert!(log_contents.contains("codex_native_summary"));
