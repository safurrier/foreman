...
#[test]
fn bootstrap_prefers_pi_native_signal_and_falls_back_to_compatibility() {
    let fixture = TmuxFixture::new();
    let pane_id = fixture.new_session("alpha", &fixture.shell_command("Pi is thinking"));
    ...
    assert_eq!(first.pi_native.applied, 1);
    assert_eq!(first.pi_native.fallback_to_compatibility, 0);
    ...
    assert_eq!(second.pi_native.applied, 0);
    assert_eq!(second.pi_native.fallback_to_compatibility, 1);
}
...
"#,
    ...
    assert_eq!(agent.integration_mode, IntegrationMode::Compatibility);
    assert_eq!(agent.status, AgentStatus::Working);
    assert_eq!(summary.pi_native.applied, 0);
...
"#,
    ...
    assert_eq!(agent.status, AgentStatus::Working);
    assert_eq!(summary.pi_native.applied, 0);
    assert_eq!(summary.pi_native.fallback_to_compatibility, 1);
    assert!(summary.pi_native.warnings.is_empty());
}

#[test]
fn binary_bootstrap_logs_pi_native_summary() {
    let fixture = TmuxFixture::new();
    let pane_id = fixture.new_session("alpha", &fixture.shell_command("Pi is thinking"));
    fixture.wait_for_capture(&pane_id, "Pi is thinking");
    ...
    assert!(log_contents.contains("pi_native_summary"));
