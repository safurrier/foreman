---
id: plan-todo
title: Task List
description: >
  Checkable tasks for this unit of work. Check off as you go.
  See _example/ for a reference.
---

# TODO — native-warning-root-fix

- [x] Add runtime command metadata to tmux pane records and app panes.
- [x] Prefer runtime identity for Claude/Codex/Pi recognition.
- [x] Preserve stale native signal protection across harnesses.
- [x] Keep native `working` hook status from being overridden by terminal text.
- [x] Add pane-level doctor evidence for missing native signal warnings.
- [x] Add simulated tmux E2E coverage for false Claude warnings and native purity.
- [x] Run focused native/tmux tests.
- [x] Run `mise run check`.
